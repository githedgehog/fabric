...
| `portLocators` _object (keys:string, values:string)_ | PortLocators is a map of port locator LED expire times which could be specified as 10m for 10 minutes (between 1 and 20 minutes),<br />empty for default 5 minutes or exact expiry time in "2026-08-05 18:45:47" (UTC) format, key is the port name or<br />"*" to apply to all ports of the switch (in which case any per-port entries are dropped) |  |  |
