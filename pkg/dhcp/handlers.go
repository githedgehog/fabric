	...
	return func(args ...string) (handler.Handler4, error) {
		return func(req, resp *dhcpv4.DHCPv4) (*dhcpv4.DHCPv4, bool) {
			...
			if relayAgentInfo := req.RelayAgentInfo(); relayAgentInfo != nil {
				vrf = strings.ToLower(string(relayAgentInfo.Get(dhcpv4.VirtualSubnetSelectionSubOption)))
				if len(vrf) > 1 {
					vrf = vrf[1:]
				}
				...
				circuitID = strings.ToLower(string(relayAgentInfo.Get(dhcpv4.AgentCircuitIDSubOption)))
				remoteID = strings.ToLower(string(relayAgentInfo.Get(dhcpv4.AgentRemoteIDSubOption)))
			}
			...
			if strings.HasPrefix(remoteID, cumulusRemoteIDPrefix) {
				for _, some := range s.subnets {
					if some.Spec.CircuitID == circuitID {
						if ok && subnet != nil {
							slog.Warn("Duplicate subnet found for Cumulus dhcp-relay, skipping", "subnet", some.Name, "other", subnet.Name, "vrf", vrf, "circuitID", circuitID, "remoteID", remoteID)
