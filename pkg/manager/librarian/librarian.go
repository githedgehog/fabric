...
func (m *Manager) CatalogForExternals(ctx context.Context, kube kclient.Client, ret *agentapi.CatalogSpec, extAttachments map[string]bool) error {
	...
	for name := range extAttachments {
		...
		if !exists {
			// Deliberately fatal: a deletion between the attachment list and this catalog read
			// re-triggers every switch's reconcile anyway, and skipping the attachment would leave
			// an ExternalPeering whose External has no attachment - which nil-derefs the agent's
			// planExternalPeerings.
