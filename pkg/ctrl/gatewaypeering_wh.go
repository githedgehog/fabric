...
func (w *PeeringWebhook) Default(_ context.Context, peer *gwapi.GatewayPeering) error {
	peer.Default()

	return nil
}

func (w *PeeringWebhook) ValidateCreate(ctx context.Context, peer *gwapi.GatewayPeering) (admission.Warnings, error) {
	return nil, peer.Validate(ctx, w.Reader) //nolint:wrapcheck
}

func (w *PeeringWebhook) ValidateUpdate(ctx context.Context, _ *gwapi.GatewayPeering, newPeer *gwapi.GatewayPeering) (admission.Warnings, error) {
	// TODO validate diff between oldObj and newObj if needed

	return nil, newPeer.Validate(ctx, w.Reader) //nolint:wrapcheck
}

func (w *PeeringWebhook) ValidateDelete(_ context.Context, _ *gwapi.GatewayPeering) (admission.Warnings, error) {
	return nil, nil
}
