...
type PeeringSpec struct {
	...
	// ACL is an optional, peering-scoped ACL implemented in the gateway
...
const (
	...
	ACLMatchProtocolICMP        ACLMatchProtocol = "icmp"
...
type PeeringACLMatch struct {
	...
	// Protocol to match ("tcp", "udp", "icmp" or numeric), omit to match any protocol
...
type PeeringACLRule struct {
	...
	// What the rule should match against, omit to match all flows in the rule's direction
...
type PeeringACL struct {
	// Default action to execute if no rules matches
