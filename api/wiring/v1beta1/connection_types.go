		...
		if conn.Spec.Fabric != nil { //nolint:gocritic
			...
			for idx, link := range cm.Links {
				...
				if leaf1Prefix.Masked() != leaf2Prefix.Masked() {
					return nil, errors.Errorf("mesh connection %s link %d spine IP %s and leaf IP %s are not in the same subnet", conn.Name, idx, leaf1IP, leaf2IP) //nolint:goerr113
				}
			}
			...
			for idx, link := range cg.Links {
				...
				if switchPrefix.Masked() != gwPrefix.Masked() {
					return nil, errors.Errorf("gateway connection %s link %d spine IP %s and gateway IP %s are not in the same subnet", conn.Name, idx, switchIP, gwIP) //nolint:goerr113
				}
			}
		}
	}
